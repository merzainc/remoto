...
function getExpoTheme(extend = {}, plugins = []) {
  ...
  customizedTheme.theme.extend = Object.assign(
    {},
    merge(expoTheme.theme.extend, extend)
  );
  ...
  ...getExpoTheme(
    {
      backgroundImage: (theme) => ({
        ...
        'default-fade-down': `linear-gradient(to bottom, transparent, ${theme(
          'backgroundColor.default'
        )})`,
        appjs:
          "url('/static/images/appjs.svg'), linear-gradient(#0033cc, #0033cc)",
      }),
      fontSize: {
        inherit: [
          'inherit',
